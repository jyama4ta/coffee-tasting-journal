...
### ローカル環境でPlaywrightが実行できない場合

**dev container等でPlaywrightが実行できない環境の場合:**

```
1. ユーザーに「ローカル環境でE2Eテストが実行できません」と報告
2. テストファイルの内容を確認し、変更が既存テストに影響しないか検証
3. ユーザーの許可を得た場合のみコミットを実行
4. コミットメッセージに「E2Eテスト未実行（環境制約）」と明記
```

**注意: これは例外措置であり、可能な限りE2Eテストを実行すること**
