...
export default function EditDripperPage({ params }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/admin/drippers/${dripper.id}`);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : "エラーが発生しました");
    } finally {
  ...
  if (!dripper) {
    return (
      <div className="max-w-2xl mx-auto text-center py-8">
        ...
        <Button href="/admin/drippers" variant="outline" className="mt-4">
          ドリッパー一覧に戻る
        </Button>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-6">
        ...
        <span>/</span>
        <Link href="/admin/drippers" className="hover:text-gray-900">
          ドリッパー一覧
        </Link>
        <span>/</span>
        <Link
          href={`/admin/drippers/${dripper.id}`}
      ...
      >
        ...
        <div className="flex gap-4">
          ...
          <Button href={`/admin/drippers/${dripper.id}`} variant="outline">
            キャンセル
          </Button>
        </div>
      </form>
